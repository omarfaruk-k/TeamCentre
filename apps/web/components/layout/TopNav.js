            ...
            <div className="hidden md:flex -space-x-2">
              {onlineMembers.slice(0, MAX_SHOWN).map((m) => (
                <div
                  key={m.user.id}
                  title={m.user.name}
                  className="w-7 h-7 rounded-full border-0,7 flex items-center justify-center text-xs font-bold text-[var(--text)] ring-1 ring-black"
                  style={{ backgroundColor: accent, borderColor: 'var(--accent)' }}
                >
                  {m.user.avatarUrl ? (
